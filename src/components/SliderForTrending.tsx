...
// use
import { getTrendingContents, IContent, IGetContentsResult } from "../api";
...
function SliderForTrending({title, content, hasBanner}: ISlider) {
    ...
    return (<>
        {isLoading ? (
            ...
            {/* -------------------------[1. Banner]------------------------ */}
            {hasBanner && <Banner bannerData={BannerContent as IContent} keyName={content} content="trending" />}
            {/* --------------------[2. Contents Slider]-------------------- */}
            <SliderContainer>
                <SliderBox>
        ...
        <ModalForTrending content={content} />   
    </>);
}

export default SliderForTrending;